...
RSpec.describe Budget::Amount, type: :model do
  ...
  describe 'after_create callbacks' do
    describe '.set_default_amount!' do
      ...
      let(:item) { FactoryBot.create(:item, default_amount: default_amount) }
      let(:budget_amount) do
        FactoryBot.create(:budget_amount, amount: amount, item: item)
      end
    ...
    describe '.set_month' do
      ...
      let(:budget_amount) { FactoryBot.create(:budget_amount, month: month) }
  ...
  describe 'amount validation' do
    subject { FactoryBot.build(:budget_amount, amount: amount) }
    context 'item is an expense' do
      let(:item) { FactoryBot.create(:item, expense: true) }
    ...
    context 'item is a revenue' do
      let(:item) { FactoryBot.create(:item, expense: false) }
      context "budget amount's amount is < 0" do
        let(:amount) { -100 }
        it { should be_valid }
      end
...
RSpec.describe Budget::MonthlyAmount, type: :model do
  ...
  let!(:rent) { FactoryBot.create(:item, monthly: true, expense: true) }
  let!(:current_rent) { FactoryBot.create(:budget_amount, item: rent, amount: -900) }
  ...
  describe '#remaining' do
    ...
    context 'items have been paid' do
      let(:cmm_income) { FactoryBot.create(:item, monthly: true, expense: false) }
      before do
        FactoryBot.create(:budget_amount, item: cmm_income, amount: 2000)
        FactoryBot.create(:transaction, account: account, budget_amount: current_rent,
...
RSpec.describe Budget::WeeklyAmount, type: :model do
  describe 'class methods' do
    let(:grocery) { FactoryBot.create(:item, expense: true, monthly: false, name: 'Grocery') }
    let(:gas) { FactoryBot.create(:item, expense: true, monthly: false, name: 'Gas') }
    let(:budget_amounts) do
      [ FactoryBot.create(:weekly_amount, item: grocery),
        FactoryBot.create(:weekly_amount, item: gas) ]
    end
    describe '#remaining' do
      before do
  ...
  describe '.remaining' do
    ...
    let(:budget_amount) { FactoryBot.create(:weekly_amount, item: item, amount: amount) }
    before do
      amounts.each { |amt| FactoryBot.create(:transaction, amount: amt, monthly_amount_id: budget_amount.id) }
    end
    ...
    context 'revenue' do
      let(:amount) { 1000 }
      let(:item) do
        FactoryBot.create(:item, expense: false, monthly: false, name: 'Lyft')
      end
    ...
    context 'expense' do
      let(:amount) { -1000 }
      let(:item) { FactoryBot.create(:item, expense: true, monthly: false, name: 'Grocery') }
